...
const TestApp: React.FunctionComponent<TestAppProps> = ({
  children,
  clientKey,
  sessionToken,
}) => {
...
describe.skip("Live Test", () => {
  ...
  beforeAll(async () => {
    ...
    pricingTier = await warrant.PricingTier.create({
      pricingTierId: "test-pricing-tier",
    });
    user = await warrant.User.create({ userId: "test-user" });
    sessionToken = await warrant.Session.createAuthorizationSession({
      userId: user.userId,
    });
